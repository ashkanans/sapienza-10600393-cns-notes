...
\begin{document}
	...
	\begin{enumerate}
	\item Define a secret key (seed)
	\item 	Using the seed, generate a byte stream (A.K.A. \textbf{Keystream}): i-th byte is a function of:
	\begin{enumerate}
	\item 	only the key (\textit{synchronous} stream cipher)
	\item 	both the key and first i-1 bytes of cipher text (\textit{asynchronous} stream cipher)
	\end{enumerate}
	\item 	Obtain the ciphertext by bitwise XORing plaintext and keystream
	\end{enumerate}}
